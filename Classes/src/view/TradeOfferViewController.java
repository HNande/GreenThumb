...
public class TradeOfferViewController {
   ...
   */
  public void setMemberViewController(
  ...
  @FXML public void initialize() {
    ...
    proposer.setCellValueFactory(cellData -> {
      ...
      if (proposerMember != null) {
        String fullName =
            proposerMember.getFirstName() + " " + proposerMember.getLastName();
        ...
        TextFieldTableCell.forTableColumn(new IntegerStringConverter() {
          @Override public Integer fromString(String value) {
            if (value == null || value.trim().isEmpty()) {
              ControllerHelper.showErrorMessage("Invalid or Empty Cost", "Input was left empty. Value reverted.");
              return null;
            }
            try {
              ...
              showErrorMessage("Input Error", "Cost must be a whole number. Input failed.");
              return null;
            }
          }
        }));
    ...
    name.setOnEditCommit(event -> {
      ...
      if (isNullOrEmpty(newValue)) {
        ...
        tradeOffer.setName(oldValue);
          ...
          || newValue.trim().length() > 32) {
        showErrorMessage("Name length error", "Name must be between 4 and 32 characters.");
        tradeOffer.setName(oldValue);
      } else {
    ...
    description.setOnEditCommit(event -> {
      ...
      if (isNullOrEmpty(newValue)) {
        ...
        showErrorMessage("Description length error", "Description cannot be more than 60 characters.");
        tradeOffer.setDescription(oldValue);
      } else {
    ...
    if (e.getSource() == deleteButton) {
      if (selectedTradeOffer != null) {
        if (showConfirmationMessage("Deletion confirmation", "Do you really want to delete: " + selectedTradeOffer.getName() + "?")) {
          ...
          showWarningMessage("Delete successful", "Trade Offer has been deleted successfully");
        }
      } else {
